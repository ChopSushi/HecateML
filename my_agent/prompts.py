...
def verifier_instruction() -> str:
    ...
    return (
        ...
        "## Sources\n"
        "Numbered bibliography of ONLY real retrieved sources, one per [n], each: "
        "source name, title, URL, and the accessed_at timestamp, e.g.:\n"
        "[1] Wikipedia — 'Basel III'. https://en.wikipedia.org/wiki/Basel_III "
        "(accessed 2026-06-25T..Z)\n\n"

        "CITATION INTEGRITY (strict — do not violate):\n"
        "- A source is ONLY a successful tool result that returned content (a "
        "Wikipedia article, an arXiv paper, a FRED series). Cite ONLY those.\n"
        "- Use ONLY the EXACT accessed_at timestamp that appeared in that tool "
        "result. NEVER invent, guess, or back-date a timestamp. If a result has "
        "no accessed_at, omit the date — do not fabricate one.\n"
        "- A tool ERROR (e.g. 'arXiv search failed: ReadTimeout'), a 'note', a "
        "budget rejection, or the researcher's own text are NOT sources. Never "
        "give them a [n] or a bibliography entry. If a tool failed, mention the "
        "failure in the Answer prose, not as a citation.\n"
        "- If, after verification, there are NO real retrieved sources (e.g. an "
        "out-of-scope refusal), OMIT the ## Sources section entirely rather than "
        "inventing one.\n\n"

        "If the draft is an out-of-scope refusal (no facts to verify), pass it "
        "through as the final answer with NO invented facts, citations, or "
        "sources — omit the ## Facts and ## Sources sections.\n\n"
